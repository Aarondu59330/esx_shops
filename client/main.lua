...
Citizen.CreateThread(function()
	while ESX == nil do
		TriggerEvent('::{AstraRP}::esx:getSharedObject', function(obj) ESX = obj end)
		Citizen.Wait(10)
	end
	...
	ESX.TriggerServerCallback('::{AstraRP}::esx_shops:requestDBItems', function(dbItems)
		shopItems = dbItems
	end)
end)

AddEventHandler('::{AstraRP}::esx_shops:hasEnteredMarker', function()
	CurrentAction = 'shop_menu'
	CurrentActionMsg = _U('press_menu')
end)

AddEventHandler('::{AstraRP}::esx_shops:hasExitedMarker', function()
	CurrentAction = nil
	RageUI.Visible(RMenu.Get('rageui', 'shop'), false)
end)

RegisterNetEvent('::{AstraRP}::esx_shops:removePickup')
AddEventHandler('::{AstraRP}::esx_shops:removePickup', function(bank)
	for i = 1, #objects do
...
RegisterNetEvent('::{AstraRP}::esx_shops:robberyOver')
AddEventHandler('::{AstraRP}::esx_shops:robberyOver', function()
	Citizen.Wait(10000)
	robbing = false
end)

RegisterNetEvent('::{AstraRP}::esx_shops:talk')
AddEventHandler('::{AstraRP}::esx_shops:talk', function(store, text, time)
	robbing = false
...
RegisterNetEvent('::{AstraRP}::esx_shops:rob')
AddEventHandler('::{AstraRP}::esx_shops:rob', function(store)
						...
						while true do
							...
							if DoesEntityExist(object) then
								if #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(object)) <= 1.5 then
									...
									_TriggerServerEvent('::{AstraRP}::esx_shops:pickUp', store)
									break
								end
							else
								break
...
RegisterNetEvent('::{AstraRP}::esx_shops:resetStore')
AddEventHandler('::{AstraRP}::esx_shops:resetStore', function(store)
...
RegisterNetEvent('::{AstraRP}::esx_shops:msgPolice')
AddEventHandler('::{AstraRP}::esx_shops:msgPolice', function(store)
	...
	while true do
		...
		for i = 1, #Config.Zones, 1 do
			if Config.Zones[i].ped.handle and IsPedDeadOrDying(Config.Zones[i].ped.handle) then
				_TriggerServerEvent('::{AstraRP}::esx_shops:pedDead', i)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('::{AstraRP}::esx_shops:hasEnteredMarker')
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('::{AstraRP}::esx_shops:hasExitedMarker')
		end
	end
end)
			...
			for i = 1, #Config.Zones do
				if Config.Zones[i].ped.handle then
					...
					if HasEntityClearLosToEntityInFront(plyPed, shopPed, 19) and not IsPedDeadOrDying(shopPed) and #(plyCoords - GetEntityCoords(shopPed)) <= 5.0 then
						if not robbing then
							ESX.TriggerServerCallback('::{AstraRP}::esx_shops:canRob', function(enoughCops, canRob)
								if enoughCops and canRob then
									...
									if #(GetEntityCoords(plyPed) - GetEntityCoords(shopPed)) <= 7.5 then
										if not IsPedDeadOrDying(shopPed) then
											_TriggerServerEvent('::{AstraRP}::esx_shops:rob', i)
								...
								elseif enoughCops and not canRob then
									TriggerEvent('::{AstraRP}::esx_shops:talk', i, 'Je me suis déjà fais volé la caisse est vide', 5)
									Citizen.Wait(2500)
	...
	RageUI.IsVisible(RMenu.Get('rageui', 'shop'), true, false, true, function()
		for i = 1, #shopItems, 1 do
			RageUI.Button(shopItems[i].label, '', {RightLabel = ('%s$'):format(shopItems[i].price)}, true, function(Hovered, Active, Selected)
				if Selected then
					_TriggerServerEvent('::{AstraRP}::esx_shops:buyItem', shopItems[i].value, 1, CurrentZone)
				end
			end)
		end
	end, function()
